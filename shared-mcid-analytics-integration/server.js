...
function executeBatch(res, offer, targetClient, visitor, targetCookie) {
  const payload = {
    mboxes: [
      {
  ...
  const batchRequest = Object.assign({payload}, {visitor}, {targetCookie});
  targetClient.getOffers(batchRequest)
    .then(batchOffer => {
      const allOffers = {
        singeOffer: offer,
        batchOffers: batchOffer,
        targetCookie: batchOffer.targetCookie,
        visitorState: batchOffer.visitorState
      };
      sendResponse(res, allOffers);
    })
    .catch(error => {
...
app.get("/", function (req, res) {
  ...
  const request = Object.assign({payload}, {visitor}, {targetCookie});
  ...
  .then(offer => {
    executeBatch(res, offer, marketingCloudClient, visitor, targetCookie);
  })
  .catch(error => {
    sendResponse(res, error);
  });
